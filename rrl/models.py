...
class MLLP(nn.Module):
    def __init__(self, dim_list, i_min, i_max, t1, t2, interval, interval2, use_not=False, left=None, right=None, estimated_grad=False):
        ...
        for i in range(1, len(dim_list)):
            ...
            if i == 1:
                layer = BinarizeLayer(num, self.i_min, self.i_max, self.t1, self.t2, self.interval, self.interval2, self.use_not)
    ...
    def binarized_forward(self, x):
        with torch.no_grad():
            ...
            for i, layer in enumerate(self.layer_list):
                if i == 0:
                    x, kmeans_loss = layer.binarized_forward(x)
                elif i == 1:
...
class RRL:
    def __init__(self, dim_list, device_id, i_min, i_max, t1, t2, interval, interval2, lambda_, use_not=False, is_rank0=False, log_file=None, writer=None,
                 left=None, right=None, save_best=False, estimated_grad=False, save_path=None, distributed=True):
        ...
        self.net = MLLP(dim_list, i_min, i_max, t1, t2, interval, interval2, use_not=use_not, left=left, right=right, estimated_grad=estimated_grad)
        
        rrl_args = {'dim_list': self.dim_list, 'use_not': self.use_not, 'estimated_grad': self.estimated_grad}
        torch.save({'model_state_dict': self.net.state_dict(), 'rrl_args': rrl_args}, self.save_path[:-4] + '_init.pth')
    ...
    def train_model(self, X=None, y=None, X_validation=None, y_validation=None, data_loader=None, valid_loader=None,
        ...
        for epo in range(epoch):
            ...
            for X, y in data_loader:
                ...
                (y_pred_mllp, kmeans_loss_mllp), (y_pred_rrl, _) = self.net.forward(X)
                ...
                kmeans_loss_mllp = self.lambda_ * kmeans_loss_mllp
                kmeans_loss_mllp.backward()
                cnt += 1
                ...
                if self.is_rank0 and cnt % TEST_CNT_MOD == 0:
                    ...
                    if self.save_best and f1_b > self.best_f1:
                        self.best_f1 = f1_b
                        self.save_model()
                        logging.debug('best epoch: {} '.format(epo))
    ...
    def test(self, X=None, y=None, test_loader=None, batch_size=32, need_transform=True, set_name='Validation'):
        ...
        with torch.no_grad():
            ...
            for X, y in test_loader:
                ...
                y_pred_list.append(output[0][0])
                y_pred_b_list.append(output[1][0])
