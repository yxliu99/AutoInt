...
class DBEncoder:
    ...
    def transform(self, X_df, y_df, bin1, bin2, normalized=False, keep_stat=False):
        X_df = X_df.reset_index(drop=True)
        y_df = y_df.reset_index(drop=True)
        discrete_data, continuous_data = self.split_data(X_df)
        ...
        #  Initial interval and maximum minimum value
        i_min = None
        i_max = None
        interval = None
        interval2 = None
        if not continuous_data.empty:
            ...
            i_min = continuous_data.min(axis=0)
            i_max = continuous_data.max(axis=0)

            col_name = [column for column in continuous_data]
            interval = []
            for name in col_name:

                t = []
                continuous_data_sort = continuous_data.sort_values(by=[name], ascending=[True])
                continuous_data_sort = continuous_data_sort.reset_index(drop=True)
                for b in range(1, bin1):
                    t.append(continuous_data_sort[name][int(len(continuous_data_sort) / bin1 * b)])

                t_gap = []
                for i in range(len(t) + 1):
                    if i == 0:
                        t_gap.append((t[i] - i_min[name]) / 2.)
                    elif i == 1:
                        t_gap.append((t[i] - t[i - 1]) / 2. + (t[i - 1] - i_min[name]) / 2.)
                    elif i == len(t):
                        t_gap.append((i_max[name] - t[i - 1]) / 2. + (t[i - 1] - t[i - 2]) / 2.)
                    else:
                        t_gap.append((t[i] - t[i - 1]) / 2. + (t[i - 1] - t[i - 2]) / 2.)
                interval.append(t_gap)
            ...
            if bin2 != 0:
                for name in col_name:
                    ...
                    continuous_data_sort = continuous_data.sort_values(by=[name], ascending=[True])
                    continuous_data_sort = continuous_data_sort.reset_index(drop=True)
                    for b in range(1, bin2):
                        t.append(continuous_data_sort[name][int(len(continuous_data_sort) / bin2 * b)])
                    ...
                    for i in range(len(t) + 1):
                        if i == 0:
                            ...
                            t_gap.append((t[i] - t[i - 1]) / 2. + (t[i - 1] - t[i - 2]) / 2.)
                    interval2.append(t_gap)

            i_min = i_min.values.tolist()
            i_max = i_max.values.tolist()
        ...
        return X_df.values, y, i_min, i_max, interval, interval2
